...
async def create_rule(
    ...
    """创建质检规则。"""
    service = QcRuleService(auth)
    rule = await service.create_rule(session, data)
...
async def list_rules(
    ...
    """获取规则列表。"""
    service = QcRuleService(auth)
    rules = await service.list_rules(session, category, status)
...
async def create_prompt_template(
    ...
    """创建Prompt模板。"""
    service = QcRuleService(auth)
    template = await service.create_prompt_template(session, data)
    await session.commit()
...
async def update_prompt_template(
    ...
    """更新Prompt模板。"""
    service = QcRuleService(auth)
    template = await service.update_prompt_template(session, template_id, data)
    await session.commit()
...
async def list_prompt_templates(
    ...
    """获取Prompt模板列表。"""
    service = QcRuleService(auth)
    templates = await service.list_prompt_templates(session, status)
...
async def publish_rule_version(
    ...
    """发布规则版本快照。"""
    service = QcRuleService(auth)
    version = await service.publish_rule_version(session, data)
    await session.commit()
...
async def list_rule_versions(
    ...
    """获取规则版本列表。"""
    service = QcRuleService(auth)
    versions = await service.list_rule_versions(session, status)
