<div align="center">
     <h1>SmartQA Service Cloud</h1>
     <p>基于 FastApiAdmin 二开的千牛 AI 客服质检系统。</p>
</div>

## SmartQA 项目说明

当前 P0 版本只围绕千牛聊天记录质检主链路开发：千牛源库 `aizhijian` 只读同步，业务库使用 `smartqa`，老板初始账号为 `boss/admin`，AI 模型默认使用阿里 `qwen3.7-plus`。

二开业务代码放在 `backend/app/plugin/module_smartqa/` 和 `frontend/web/src/views/module_smartqa/`。FastApiAdmin 原框架的系统用户、角色、菜单、操作日志、定时任务能力优先复用；示例、订单、发票、套餐、内置 AI 聊天等非 P0 功能不进入业务菜单。

敏感资料不允许提交到 GitHub：`docs` 下的账号、密码、密钥、图片密钥文件以及本地 `.env.*` 已被 `.gitignore` 屏蔽。部署时参考 [docs/部署环境变量示例.md](docs/部署环境变量示例.md) 填入宝塔服务器真实环境变量。

关键设计文档：

- [docs/项目需求精炼总结.md](docs/项目需求精炼总结.md)
- [docs/项目系统设计精炼文档.md](docs/项目系统设计精炼文档.md)
